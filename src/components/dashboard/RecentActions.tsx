export function RecentActions({
    ...
    const cfActions = actions.filter((a) => a.provider === "cloudflare").slice(0, 5);
    const vercelActions = actions.filter((a) => a.provider === "vercel").slice(0, 5);
        ...
        <div className="bg-white border border-gray-200 rounded-md shadow-sm overflow-hidden flex flex-col h-full w-full">
                ...
                {!hasAnyActions ? (
                    ...
                    <div className="flex flex-col divide-y divide-slate-100">
                        ...
                        <div className="flex flex-col">
                            <div className="px-3 py-2 bg-slate-50 border-b border-slate-100 flex items-center gap-1.5 sticky top-0 z-10">
                                <span className="w-2 h-2 rounded-full bg-orange-500 shrink-0" />
                                <span className="text-[10px] font-black text-slate-500 uppercase tracking-wider">Cloudflare (Last 5)</span>
                            </div>
                            <div className="divide-y divide-slate-50">
                                {cfActions.length === 0 ? (
                                    <div className="py-6 text-center text-slate-400">
                                        <p className="text-xs italic text-slate-400/80">No recent actions logged.</p>
                                    </div>
                                ) : (
                                    cfActions.map(renderActionRow)
                                )}
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col">
                            <div className="px-3 py-2 bg-slate-50 border-b border-slate-100 flex items-center gap-1.5 sticky top-0 z-10">
                                ...
                                <span className="text-[10px] font-black text-slate-500 uppercase tracking-wider">Vercel (Last 5)</span>
                            </div>
                            <div className="divide-y divide-slate-50">
                                {vercelActions.length === 0 ? (
                                    <div className="py-6 text-center text-slate-400">
                                        <p className="text-xs italic text-slate-400/80">No recent actions logged.</p>
                                    </div>
                                ) : (
                                    vercelActions.map(renderActionRow)
                                )}
                            </div>
                        </div>
                    </div>
            ...
            {hasAnyActions && (
                <div className="px-6 py-3 border-t border-slate-50 bg-slate-50/30 flex justify-center">
                    ...
                    >
                        View Full History
                        <svg className="w-3.5 h-3.5 transition-transform group-hover:translate-x-0.5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><line x1="5" y1="12" x2="19" y2="12"></line><polyline points="12 5 19 12 12 19"></polyline></svg>
                    </a>
                </div>
            )}
        </div>
