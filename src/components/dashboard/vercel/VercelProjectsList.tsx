...
import { toggleVercelProjectStatus, deleteVercelProject, toggleVercelRuleStatus, deleteVercelRule, toggleVercelProjectArchiveStatus } from "~/server/vercel";
...
export function VercelProjectsList({
    ...
    const handleAction = async (e: React.FormEvent, intent: string, params: Record<string, string>) => {
        ...
        try {
            ...
            } else if (intent === "toggle_vercel_project_archive") {
                res = await toggleVercelProjectArchiveStatus(params.projectId, params.isArchived === "true");
            }
    ...
    const renderProjectRow = (project: any) => {
        ...
        return (
            <div key={project.id} className={`px-3 sm:px-5 py-4 border-b border-gray-100 last:border-0 ${project.isArchived ? 'bg-slate-50/50 opacity-80' : 'bg-gradient-to-br from-white to-gray-50/30'}`}>
                <div className="flex flex-wrap sm:flex-nowrap items-center gap-2 sm:gap-3">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-4 order-last sm:order-none w-full sm:w-auto sm:flex-1 min-w-0">
                        <div className="flex flex-col min-w-0 shrink-0">
                            <span className="text-[9px] uppercase font-black tracking-wider text-slate-400/80 leading-none mb-1">Project Name</span>
                            <span className="text-sm font-black text-slate-900 truncate" title={project.name}>{project.name}</span>
                        </div>
                        <div className="flex flex-col min-w-0 shrink-0">
                            <span className="text-[9px] uppercase font-black tracking-wider text-slate-400/80 leading-none mb-1">Domain</span>
                            <span className="text-xs font-mono font-semibold text-slate-600 truncate" title={project.domain || "—"}>{project.domain || "—"}</span>
                        </div>
                        <div className="flex flex-col min-w-0 shrink-0">
                            <span className="text-[9px] uppercase font-black tracking-wider text-slate-400/80 leading-none mb-1">Project ID</span>
                            <span className="text-xs font-black text-indigo-600 truncate" title={project.vercelProjectId}>
                    ...
                    <div className="flex items-center gap-1.5 shrink-0 ml-auto sm:ml-0">
                        ...
                        <form onSubmit={(e: React.FormEvent) => {
                            e.preventDefault();
                            const message = project.isArchived
                                ? `Unarchive project "${project.name}"?`
                                : `Archive project "${project.name}"?`;
                            if (confirm(message)) {
                                handleAction(e, "toggle_vercel_project_archive", { projectId: project.id, isArchived: (!project.isArchived).toString() });
                            }
                        }}>
                            <button
                                type="submit"
                                disabled={submittingId === `toggle_vercel_project_archive-${project.id}`}
                                className="p-1.5 text-slate-500 hover:text-indigo-600 bg-white hover:bg-slate-50 border border-slate-200 hover:border-indigo-200 rounded-md transition-all shadow-sm shrink-0"
                                title={project.isArchived ? "Unarchive Project" : "Archive Project"}
                            >
                                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className={project.isArchived ? "text-amber-500" : ""}>
                        ...
                        <form onSubmit={(e: React.FormEvent) => {
                            ...
                            <button type="submit" disabled={submittingId === `delete_vercel_project-${project.id}`} className="p-1.5 text-rose-500 hover:text-rose-600 bg-white hover:bg-rose-50 border border-slate-200 hover:border-rose-200 rounded-md transition-all shadow-sm disabled:opacity-50 disabled:cursor-not-allowed shrink-0">
        ...
        <div className="bg-white border border-gray-200 rounded-md shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-100 bg-gray-50/50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <div className="flex items-center justify-between sm:justify-end gap-3 w-full sm:w-auto">
                    <span className="text-[10px] font-black text-slate-800 uppercase tracking-widest bg-slate-100 px-3 py-1 rounded-full">{activeProjects.length} Active</span>
                    {hasAccounts ? (
                        <button
                ...
                {activeProjects.length === 0 ? (
                    <div className="py-16 flex flex-col items-center justify-center text-center gap-3">
                        ...
                        <p className="text-black font-medium text-sm">
                            {hasAccounts ? "No active Vercel projects registered yet." : "No Vercel accounts connected yet."}
                        </p>
                        {hasAccounts ? (
                ...
                <div className="border-t border-gray-200/80 bg-slate-50/20">
                    ...
                    >
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm font-bold text-slate-700">Archived Projects</span>
                            <span className="text-xs font-semibold text-slate-500 bg-slate-100 px-2 py-0.5 rounded-md">{archivedProjects.length}</span>
                        </div>
                        <svg
...
function RuleDetails({ rule }: { rule: any }) {
    if (rule.type === "vercel_under_attack_mode") {
        return (
            <div className="grid grid-cols-2 md:grid-cols-[1.5fr_1.5fr_1fr_1.2fr_0.8fr_2fr] gap-2 sm:gap-4 flex-1 min-w-0">
                ...
                <div className="flex flex-col min-w-0 shrink-0">
                    <span className="text-[9px] uppercase font-black tracking-wider text-slate-400/80 leading-none mb-1">Source / Target</span>
                    <span className="text-xs font-black text-slate-800 truncate leading-tight" title={rule.trafficSource === "cloudflare" ? "CF Analytics" : "Log Drain"}>
    ...
    if (rule.type === "vercel_bot_protection") {
        return (
            <div className="grid grid-cols-2 md:grid-cols-[1.5fr_1.5fr_1fr_1.2fr_0.8fr_2fr] gap-2 sm:gap-4 flex-1 min-w-0">
                <div className="flex flex-col min-w-0 shrink-0">
                    <span className="text-[9px] uppercase font-black tracking-wider text-purple-500/80 leading-none mb-1">Bot Protection</span>
                    <span className="text-sm font-black text-slate-900 truncate leading-tight" title={rule.name}>{rule.name}</span>
                </div>
                <div className="flex flex-col min-w-0 shrink-0">
                    <span className="text-[9px] uppercase font-black tracking-wider text-slate-400/80 leading-none mb-1">Source & Action</span>
                    <span className="text-xs font-black text-slate-800 truncate leading-tight">
                        {rule.trafficSource === "cloudflare" ? "CF" : "Drain"} ({rule.action})
                    </span>
                </div>
                <div className="flex flex-col min-w-0 shrink-0">
                    <span className="text-[9px] uppercase font-black tracking-wider text-slate-400/80 leading-none mb-1">ON Trigger</span>
