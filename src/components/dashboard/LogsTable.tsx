        ...
        <div className="relative overflow-x-auto custom-scrollbar bg-white border border-slate-200 shadow-sm ">
            <table className="w-full min-w-max text-left border-collapse">
                <thead className="bg-slate-50/95 backdrop-blur-sm border-b border-slate-100 sticky top-0 z-10">
                    <tr>
                        {onToggleSelectAll && (
                            <th className="pl-6 pr-2 py-5 w-[48px] text-center">
                                <input
                                    type="checkbox"
                                    checked={allSelected}
                        ...
                        <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap">Resource</th>
                ...
                <tbody className="divide-y divide-slate-100">
                    {logs.map((log) => {
                        ...
                        const resourceName = log.provider === "vercel"
                            ? projects.find((p: any) => p.id === log.resourceId)?.name || "Unknown Project"
                        ...
                        return (
                            <tr key={log.id} className={`transition-all group cursor-pointer hover:bg-slate-50/80 ${isError ? 'bg-rose-50/10' : ''}`}>
                                ...
                                <td className="px-6 py-5">
                                    <span className="text-[11px] font-bold text-slate-600 uppercase tracking-wider">{resourceName}</span>
                                </td>
                                <td className="px-6 py-5">
