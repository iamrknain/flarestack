...
export function ModalShell({ onClose, icon, iconBg, title, subtitle, children }: {
    ...
    return (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto" onClick={onClose}>
            <div className="bg-white rounded-md shadow-2xl w-full max-w-lg relative flex flex-col max-h-[90vh] overflow-hidden my-auto" onClick={(e) => e.stopPropagation()}>
...
export function FormActions({ onClose, isSubmitting, submitLabel, disabled = false }: {
    onClose: () => void;
    ...
    return (
        <div className="flex items-center gap-3 pt-2 border-t border-gray-100">
            ...
            <button type="submit" disabled={isSubmitting || disabled} className="flex-1 py-2.5 rounded-md text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 transition-colors disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2">
                {isSubmitting ? (
                    <>
